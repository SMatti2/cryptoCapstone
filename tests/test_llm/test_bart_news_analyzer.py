...
class TestUniversalSentimentAnalyzer(unittest.TestCase):
    def setUp(self):
        self.analyzer = UniversalSentimentAnalyzer(
            text_columns=["title", "subtitle"],
            hypotheses=["This text is bullish.", "This text is bearish."],
            device="cpu",
            batch_size=4,
        )

    def test_prepare_texts(self):
        data = {"title": ["Title1", "Title2"], "subtitle": ["Subtitle1", "Subtitle2"]}
        df = pd.DataFrame(data)
        expected_output = ["Title1. Subtitle1", "Title2. Subtitle2"]
        self.assertEqual(self.analyzer._prepare_texts(df), expected_output)
    ...
    def test_analyze_articles(self):
        ...
        end_date = "2023-03-16"
        result = self.analyzer.analyze(df, start_date, end_date)
    ...
    def test_analyze_articles_empty_result(self):
        ...
        self.assertEqual(list(result.columns), ["date", "score"])
    ...
    def test_analyze_articles_with_output_file(self):
        ...
        try:
            analyzer_with_output = UniversalSentimentAnalyzer(
